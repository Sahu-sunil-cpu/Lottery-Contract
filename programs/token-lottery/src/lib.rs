...
declare_id!("CVNNFuwGZW7vDVJGFU2j5FK2837rUFJtcfke3vNsyoXr");
...
pub mod token_lottery {
    ...
    pub fn initialize_config(ctx: Context<Initialize>, start: u64, end: u64) -> Result<()> {
        ctx.accounts.token_lottery.bump = ctx.bumps.token_lottery;
        ctx.accounts.token_lottery.authority = ctx.accounts.payer.key();
        ctx.accounts.token_lottery.start_time = start;
        ctx.accounts.token_lottery.end_time = end;
        ctx.accounts.token_lottery.lottery_pot_amount = 0;
        ctx.accounts.token_lottery.ticket_price = 0;
    ...
    pub fn buy_ticket(ctx: Context<BuyTicket>) -> Result<()> {
        ...
        if clock.slot < ctx.accounts.lottery_token.start_time || 
        clock.slot > ctx.accounts.lottery_token.end_time {
            return Err(ErrorCode::LotteryNotOpen.into());
        }

        let ticket_name = NAME.to_owned() + ctx.accounts.lottery_token.ticket_num.to_string().as_str();
        ...
        system_program::transfer(
            CpiContext::new(
                ...
                system_program::Transfer {
                    ...
                    to: ctx.accounts.lottery_token.to_account_info()
                }
                ),
             ctx.accounts.lottery_token.ticket_price,
            )?;

            ctx.accounts.lottery_token.lottery_pot_amount += ctx.accounts.lottery_token.ticket_price;
            ...
            create_master_edition_v3(
                CpiContext::new_with_signer(
                    ...
                    CreateMasterEditionV3 {
                        payer: ctx.accounts.payer.to_account_info(),
                        mint: ctx.accounts.ticket_mint.to_account_info(),
                        edition: ctx.accounts.mater_edition.to_account_info(),
                        mint_authority: ctx.accounts.collection_mint.to_account_info(),
                        update_authority: ctx.accounts.collection_mint.to_account_info(),
                        metadata: ctx.accounts.metadata.to_account_info(),
                        token_program: ctx.accounts.token_program.to_account_info(),
            ...
            ctx.accounts.lottery_token.ticket_num += 1;
...
pub struct Initialize<'info> {
    ...
    #[account(
        init,
        payer = payer,
        space = 8 + LotteryToken::INIT_SPACE,
        seeds = [b"token_lottery".as_ref()],
        bump
    )]
    pub token_lottery: Account<'info, LotteryToken>,
...
pub struct BuyTicket<'info> {
  ...
  bump,
    ...
    pub lottery_token: Account<'info, LotteryToken>,
   
   #[account(
    init,
    payer = payer,
    seeds = [lottery_token.ticket_num.to_le_bytes().as_ref()],
    bump,
    mint::decimals = 0,
    mint::authority = collection_mint,
    mint::freeze_authority = collection_mint,
    ...
    ///CHECK: will be initialized by the metaplex program 
    pub metadata: UncheckedAccount<'info>,
    ...
    ///CHECK: will be initialized by the metaplex program 
    pub mater_edition: UncheckedAccount<'info>,
